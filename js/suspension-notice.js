...
(function () {
    ...
    function setupDismissalSystem() {
        ...
        noticeElement.setAttribute('aria-label', 'Suspension warning - Press Escape or click X to dismiss');
    }
    ...
    function createDismissModal(options) {
        ...
        options.forEach(option => {
            ...
            if (isCancel) {
                ...
                borderStyle = isNightTheme ? '1px solid rgba(98, 116, 164, 0.3)' : '1px solid #d1d5db';
            } else {
            ...
            button.addEventListener('mouseenter', () => {
                if (option.action !== 'cancel') {
                    ...
                    button.style.background = document.body.classList.contains('night-theme') ? 'rgba(124, 203, 255, 0.1)' : '#f9fafb';
                }
            });
    ...
    window.ajSuspensionNotice = {
        ...
        show: () => {
            noticeElement.classList.remove('aj-hidden', 'aj-minimized', 'aj-permanently-hidden');
            localStorage.removeItem(STORAGE_KEY);
            localStorage.removeItem(MINIMIZED_KEY);
        },
        toggle: () => {
